...
// 可选 kinds：按页面类型过滤（如 today 传 ['todo','task']，tasks 传 ['task']，archive 传 ['archive']），
// 避免三页家庭空间渲染同一份全量聚合流。
const sync = require('./sync/index.js');
const family = require('./family.js');

async function loadFamilyFeed(kinds) {
  ...
  if (Array.isArray(kinds) && kinds.length) {
    items = items.filter((it) => kinds.indexOf(it.type) !== -1);
  }
